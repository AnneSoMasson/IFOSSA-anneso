---
title: "Pipeline for functional assignation of heterogeneous data"
output:
...
order_cp = c("-config. +cereal", "-config. +tree", "+config. +cereal", "+config. +tree")
order_config = c('-config.', '+config.')
order_plantcover = c('+cereal', '+tree')
...
kingdom_color = c("Bacteria"="#ed938b", "Eukaryota"="#d200be", "Fungi"="#ad9400", "Metazoa"="#00aae3")
...
# id_campaign 191 and id_sample from ANI-23-0739 to ANI-23-0798 are not in this table yet but can be added later
...
#id_campaign 192 and id_sample from ANI-23-0799 to ANI-23-0887) are not in this table yet but can be added later
...
# OPTION 2 = simplest (rang number becomes replicate number and are transformed to range from R1 to R5)
...
```{r}
# Tidy metadata
tidy_bacteria_0 <- bacteria_0

tidy_bacteria_0 <- pivot_longer(tidy_bacteria_0, cols = c(13:72), names_to =  "Reference_ADNid", values_to = "abundance", names_repair = "minimal")

tidy_bacteria_0$Reference_ADNid <- str_replace(tidy_bacteria_0$Reference_ADNid, '_.+_.+', "")

tidy_bacteria_0 <- left_join(tidy_bacteria_0, metadata_mo_1, by = "Reference_ADNid")

tidy_bacteria_0 <- tidy_bacteria_0 %>% mutate(community = "bacteria")
```
